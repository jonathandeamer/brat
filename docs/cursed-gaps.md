...
**Need:** brat must emit file bytes verbatim. The only documented stdout path is `vibez.spill`, which goes through `printf("%s", str)` — that truncates at the first NUL byte, so it cannot carry binary file contents through cleanly. There is no documented length-prefixed / byte-faithful write surface.
**Status:** partial — `vibez.spill` covers C-string-shaped writes only.
...
**Evidence:** `~/cursed/src-zig/cursed_runtime.c:7-42` — every `spill_*` helper emits via `printf` against a `const char*`, so a NUL byte in input ends the write early and there is no `(ptr, len)`-shaped stdout primitive. brat design spec marks this gap explicitly: `docs/superpowers/specs/2026-05-09-brat-design.md:55-72,192-201`.
**Brat cases blocked:** binary-nul (only). `vibez.spill` is `printf("%s", ...)` per `~/cursed/src-zig/cursed_runtime.c:7-11`, which truncates at NUL but is otherwise byte-faithful for non-NUL output (including UTF-8 in the `unicode` case and embedded `\n` in `multiline-with-eol`). The other 17 cases can emit their file content via `vibez.spill` once `dropz.read_file` exists; only NUL-bearing payloads require a length-prefixed write.
...
| tests/cases/boundary-46 | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
...
| tests/cases/empty | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
| tests/cases/good-missing-good | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, errno-surfacing, stderr-write, exit-code-control, loops, binary-expressions, assignment-statement |
| tests/cases/long-name | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
...
| tests/cases/multi-file | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
| tests/cases/multi-missing | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, errno-surfacing, stderr-write, exit-code-control, loops, binary-expressions, assignment-statement |
| tests/cases/multiline-with-eol | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
...
| tests/cases/one-line-no-eol | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
...
| tests/cases/spaced-filename | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
| tests/cases/subdir-path | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
| tests/cases/unicode | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
| tests/cases/uppercase-name | non-stdlib-imports, user-defined-functions, member-access, array-literals-and-indexing, conditionals, argv-access, file-read, loops, binary-expressions, assignment-statement |
